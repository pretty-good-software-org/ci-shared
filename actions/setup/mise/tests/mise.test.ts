...
describe("setup/mise version pin", () => {
  it("uses the self-hosted-runner-compatible release", () => {
    assert.match(action, /^        version: 2026\.7\.1$/m, "mise must use the self-hosted-runner-compatible release");
    assert.match(action, /rejects valid core Python/, "the pin must document the locked Python regression");
    assert.match(action, /URL-less npm\/pipx locks/, "the pin must document URL-less backend compatibility");
  });
});
