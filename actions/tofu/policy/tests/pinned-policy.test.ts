...
it("fetches and evaluates the exact verified commit without credentials in the repository URL", async () => {
  ...
  assert.deepStrictEqual(
    ...
    [
      ...
      `conftest test --config-file ${join(checkoutRoot, "conftest-pinned.toml")} --policy ${join(checkoutRoot, "policy")} --update  --namespace policies.s3 --namespace policies.kms --parser json --no-fail=false --no-color --quiet=false tofu/plan.json`,
    ],
