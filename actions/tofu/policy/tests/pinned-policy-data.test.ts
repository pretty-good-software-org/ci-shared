...
it("loads the immutable data directory with --data when the pinned checkout ships one", async () => {
  ...
  assert.strictEqual(
    ...
    `conftest test --policy ${join(checkoutRoot, "policy")} --data ${join(checkoutRoot, "policy", "data")} --namespace policies.s3 --quiet=false tofu/plan.json`,
...
it("omits --data when the pinned checkout has no data directory", async () => {
  ...
  assert.strictEqual(
    conftestCommand(commands),
    `conftest test --policy ${join(checkoutRoot, "policy")} --namespace policies.s3 --quiet=false tofu/plan.json`,
