...
describe("buildComment step outcomes all fields", () => {
  const fields = [
  ...
  for (const { field, label } of fields) {
    for (const outcome of ["success", "failure", "cancelled", "skipped"]) {
      it(`renders ${field}=${outcome}`, () => {
        assert.match(
          buildComment({ ...defaults, [field]: outcome }),
...
describe("buildComment markdown injection", () => {
  it("escapes triple backticks in plan text to prevent code fence breakout", () => {
    const body = buildComment({ ...defaults, plan: "resource ``` injected" });
