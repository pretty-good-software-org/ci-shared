...
const validate = require("../action.ts");
const { run } = validate;
...
describe("run command construction", () => {
  it("includes -chdir with the working directory", () => {
    ...
    run({ workingDirectory: "infra" }, exec);

    assert.match(commands[0], /-chdir=infra/, "should include -chdir with custom directory");
  });

  it("runs the validate subcommand", () => {
    const { commands, exec } = captureCommands();
    run({ workingDirectory: "tofu" }, exec);

    assert.match(commands[0], /validate$/, "should end with 'validate'");
  });
...
describe("snapshot", () => {
  it("matches expected full command with default env", () => {
    ...
    assert.strictEqual(commands[0], "tofu -chdir=tofu validate", "full command should match expected output");
  });
});
